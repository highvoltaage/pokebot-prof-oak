...
class ShinyQuotaPlugin(BotPlugin):
    ...
    def on_pokemon_caught(self, mon: Pokemon, *args, **kwargs) -> None:
        try:
            ...
            sp = (getattr(mon, "species_name", "") or "").upper()
            if not sp: return
    ...
    def _rebuild_requirements_cache(self) -> None:
        ...
        for sname in species_list:
            s_up = sname.upper()
            member_names = self._family_species_names_from_name(s_up)  # full family
    ...
    def _family_species_names_from_name(self, species_name: str) -> Set[str]:
        ...
        1) If Species.family (list[int]) exists and non-empty -> map all indices to names.
        2) Else: climb to root via evolves_from; BFS down via evolutions.
        Always include the starting species itself.
        """
        names: Set[str] = set()
    ...
    def _maybe_pause_if_quota_met(self) -> None:
        if not self.required_families_current: return
        deficits = []
    ...
    def _species_from_enc(self, enc) -> Optional[str]:
        ...
        for attr in ("current_encounter","encounter","battle","last_encounter"):
            ...
            if isinstance(nm, str) and nm: return nm.upper()
        return None
